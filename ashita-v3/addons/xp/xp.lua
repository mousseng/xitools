...
local function update_chain(tier)
    ...
    local mlvl = AshitaCore:GetDataManager():GetPlayer():GetMainJobLevel()
    local lvl = clamp_level(mlvl)

    chain_end = os.time() + chain_timers[lvl][tier]
end

ashita.register_event('incoming_text', function(mode, msg)
    local chain, exp

    chain, exp = string.match(msg, exp_chain)
    if exp ~= nil then
    ...
    chain, exp = string.match(msg, lmt_chain)
    if exp ~= nil then
        update_chain(tonumber(chain))
        record_exp(tonumber(exp))
        return false
    ...
    exp = string.match(msg, exp_earned)
    if exp ~= nil then
        update_chain(0)
        record_exp(tonumber(exp))
        return false
    ...
    exp = string.match(msg, lmt_earned)
    if exp ~= nil then
        update_chain(0)
        record_exp(tonumber(exp))
        return false
